      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">{projects.length} annotation project{projects.length !== 1 ? "s" : ""}</p>
          </div>
          <Link
            href="/dashboard/new"
        ...
        {projects.length === 0 ? (
          <div className="text-center py-24 border border-dashed border-[#2a2d3e] rounded-xl">
            ...
            <p className="text-gray-600 text-sm mt-2 mb-6">Create your first annotation project to get started</p>
            <Link href="/dashboard/new" className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-colors">
              Create Project
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((project) => (
              <Link
                ...
                className="group block bg-[#13151e] border border-[#2a2d3e] hover:border-indigo-500/50 rounded-xl p-5 transition-all hover:bg-[#1a1d27]"
              >
                {/* Header */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-semibold text-white group-hover:text-indigo-300 transition-colors line-clamp-1">
                        {project.name}
                      </h2>
                    </div>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${getProjectTypeColor(project.type as ProjectType)}`}>
