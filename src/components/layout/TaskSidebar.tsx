...
export function TaskSidebar({ tasks, currentIndex, onSelect, filter, onFilterChange }: Props) {
  ...
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col bg-[#13151e] border-r border-[#2a2d3e] overflow-hidden">
      <div className="flex-shrink-0 px-3 py-3 border-b border-[#2a2d3e] grid grid-cols-4 gap-1 text-center">
        <div>
        ...
        <div>
          ...
          <div className="text-gray-600 text-[10px] uppercase tracking-wide">Draft</div>
        </div>
        <div>
      ...
      <div className="flex-shrink-0 p-3 border-b border-[#2a2d3e]">
        <input
          value={filter}
          onChange={(e) => onFilterChange(e.target.value)}
          placeholder="Filter ID, question, answer, annotator..."
          className="w-full rounded-lg bg-[#0e0f14] border border-[#2a2d3e] px-3 py-2 text-xs text-gray-300 placeholder:text-gray-700 outline-none focus:border-indigo-500/60"
        />
        {query && (
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {filteredTasks.length === 0 ? (
          <div className="px-3 py-6 text-xs text-gray-600 text-center">No matching tasks</div>
